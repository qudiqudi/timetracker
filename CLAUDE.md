...
- `worker/` -- Cloudflare Worker for cloud sync relay. `wrangler.toml` + `src/index.js`. Deployed to `sync.qudiqudi.de`. KV namespace `SYNC_KV` stores encrypted blobs keyed by 64-char hex channel IDs. CORS restricted to `qudiqudi.github.io` + localhost. Rate-limited (30 writes/min per channel). Supports ETag/If-Match for compare-and-swap. Validates PUT body starts with `HUBI2:`.
